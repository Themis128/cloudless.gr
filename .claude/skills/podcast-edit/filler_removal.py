...
def main():
    ...
    parser.add_argument(
        "--end-at", type=float, default=None, help="Cut everything after this timestamp (seconds)"
    )
    parser.add_argument(
        "--cut",
        action="append",
    ...
    segments = build_keep_segments(
        words, is_filler, args.total_duration, end_at=args.end_at, cut_ranges=cut_ranges
    )
    ...
    print(
        f"Original duration: {args.total_duration:.1f}s ({args.total_duration / 60:.1f}min)",
        file=sys.stderr,
    )
