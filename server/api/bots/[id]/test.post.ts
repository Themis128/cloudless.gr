...
export default defineEventHandler(async (event) => {
  ...
  const { data: bot, error } = await client.from('bots').select('*').eq('id', botId).single()
  if (error || !bot) {
    return { error: 'Bot not found' }
  }

  // Find pipeline by bot_id
  const { data: pipeline, error: pipeError } = await client.from('pipelines').select('*').eq('bot_id', bot.id).order('created_at', { ascending: false }).limit(1).single()
  if (pipeError || !pipeline) {
    return { error: 'No pipeline found for this bot.' }
  }

  // Simulate step-by-step pipeline execution
  const steps = Array.isArray(pipeline.config?.steps) ? pipeline.config.steps : []
  let stepResults = []
  for (let i = 0; i < steps.length; i++) {
    ...
    stepResults.push({
      ...
      result: i === 0 ? `Step '${steps[i].name || steps[i]}' started with input: ${message}` : null
    })
  }
  return {
