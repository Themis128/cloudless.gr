...
// Helper functions
export function getPlanByPriceId(priceId: string): { key: StripePlanKey; plan: typeof STRIPE_PLANS[StripePlanKey] } | null {
...
// Customer management
export async function getOrCreateStripeCustomer(userId: string, email: string, name?: string): Promise<Stripe.Customer> {
  ...
  const existingCustomer = customers.data.find(customer => 
    customer.metadata && customer.metadata.supabase_user_id === userId
  )
