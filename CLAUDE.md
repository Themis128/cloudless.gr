...
- **Production edge = Worker `cloudless2` as proxy.** Traffic is `cloudless.gr → cloudless2 (workers/pi-origin-proxy) → pi-origin Tunnel → k3s cloudless-app on omv`. The Worker is reverse-proxy only (no app secrets). Runtime secrets live on the Pi pod. See `.cursor/rules/cloudless2-pi-proxy.mdc`.
...
| CI `CLOUDFLARE_API_TOKEN` scope    | **⚠️ ACTION NEEDED**    | The `CLOUDFLARE_API_TOKEN` GitHub secret lacks `Workers Scripts:Edit` — `deploy-postiz-ai-proxy` fails "No access to the specified resource" (found 2026-09-25). Fix: add `CF_EMAIL`+`CF_GLOBAL_API_KEY` repo secrets (dash → My Profile → Global API Key) and run `ensure-cloudflare-ci-token`, or swap the secret for a Workers-scoped token. Local `wrangler deploy` works meanwhile (the token in cu130-slim `.env` has the scope). |
...
> **Registry:** first-party image refs use `ghcr.io/themis128/cloudless-pi-app` (ECR `278585680617.../cloudless-pi-app` is retired). Do not resurrect `build-pi-image.yml` ECR builds. **Live deploy:** `deploy-pi.yml` (R2 + Cloudflare Workflow).
