...
| SafeDeploy Watchdog              | **✅ LIVE (2026-08-09)** | Continuous prod monitor on omv (systemd timer, every 2min). Alerts on ntfy+Slack+email at 6min unhealthy; auto-rollback at 16min. Complements the deploy-time rollback in SafeDeploy. See `docs/SAFEDEPLOY-WATCHDOG.md`; re-install: `sudo bash infrastructure/omv/install-safedeploy-watchdog.sh`. |
