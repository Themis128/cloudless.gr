...
Paid-acquisition landing pages live under `/<locale>/campaigns/<slug>/`. The
operating playbook — add a new campaign, wire its Stripe checkout, dual-fire
the LinkedIn conversion (Insight Tag + Conversions API) — is in
`skills/linkedin-campaigns/SKILL.md`; the architecture reference is in
`docs/linkedin-campaigns.md`. Read the skill before touching:
...
- `src/app/[locale]/campaigns/**` (landing + thanks pages)
...
unset the corresponding fire becomes a no-op — the route stays wired so
