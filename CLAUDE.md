...
| HubSpot → EspoCRM migration      | **BLOCKED — wrong key type** | Key in `.env.local` (`eu1-25ae-...`) is an OAuth token, not a private app key. Get the correct key: HubSpot → Settings → Integrations → Private Apps → your app → copy token starting `pat-eu1-...`. Confirm scopes include `crm.objects.contacts.read`, `crm.objects.companies.read`, `crm.objects.deals.read`. Then run: `HUBSPOT_API_KEY=pat-eu1-... ESPOCRM_BASE_URL=https://espocrm.cloudless.gr ESPOCRM_API_KEY=1e9f15bcd0368bce98b5de76c6929745 node scripts/migrate-hubspot-to-espocrm.mjs` |
