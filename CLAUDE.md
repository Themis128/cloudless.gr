...
| Cloudflare Email Obfuscation fix | **TOKEN NEEDED**    | `cloudflare-disable-email-obfuscation.yml` (merged PR #745) fixes React #418 hydration errors — needs a Cloudflare API token with **Zone Settings:Edit** on `cloudless.gr`. Same token that lives in `.env.local` as `CLOUDFLARE_API_TOKEN` today has the scope; the workflow just needs a cluster-side secret. |
