...
#### Tier 1 - Review First
- **PR #1272**: agentic/r21c-langsmith-observability
  - Impact: Agent system observability improvements
  - Size: 67 commits
  - Risk: Medium (new observability infrastructure)
  - Decision: Approve → Merge or Request Changes

- **PR #1273**: claude/cleanup-dead-code
  - Impact: Code cleanup and cron SSM configuration fixes
  - Size: 26 commits
  - Risk: Low (cleanup + fixes)
  - Decision: Approve → Merge

- **PR #1274**: agentic/dcode-local-setup
  - Impact: Local development environment setup
  - Size: 15 commits
  - Risk: Low (dev infrastructure)
  - Decision: Approve → Merge

#### Tier 2 - Review After Tier 1
- **PR #1276**: claude/auth-trust-host
  - Impact: next-auth configuration fix
  - Size: 6 commits
  - Risk: Low
  - Decision: Approve → Merge

- **PR #1277**: feat/weekly-newsletter
  - Impact: New newsletter feature
  - Size: 2 commits
  - Risk: Medium (new feature)
  - Decision: Review → Approve/Request Changes
...
Root Cause: Tunnel ingress port mismatch or backend unreachable  
Timeframe: This week
...
kubectl get pods | grep docs

# Check tunnel config
ssh tbaltzakis@192.168.1.128 "cat ~/.cloudflared/config.yml | grep -A5 docs"

# Update if needed and restart tunnel
ssh tbaltzakis@192.168.1.128 "sudo systemctl restart cloudflared"
```
