...
| --- | --- | --- | --- |
| **In progress** | Unified admin contact page `/admin/crm/[id]` | Highest-leverage CRM gap: EspoCRM, Stripe, and D1 are already populated; nothing joins them | Join by email. No new database. |
...
Matching is email-only. If that fails in production, *then* consider a CDP.
