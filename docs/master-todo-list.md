...
#github/workflows/r24-add-replicas.yml` adds us-west-2 replicas to DDB tables for cross-region replication. Manual failover via Cloudflare LB origin change.- [x] ~~🤖 🔵 **R24~~ ✅ **SHIPPED** — `.github/workflows/r24-add-replicas.yml` adds us-west-2 replicas to DDB tables for cross-region replication. Manual failover via Cloudflare LB origin change. R24 provides AWS-side passive DR with RPO < 1 second for shared state (orders, users, sessions)..github/workflows/r24-add-replicas.yml` adds us-west-2 replicas to DDB tables for cross-region replication. Manual failover via Cloudflare LB origin change. | R24 provides AWS-side passive DR with RPO < 1 second for shared state (orders, users, sessions).** Route 53 health-check + secondary-region Lambda (`us-west-2`) passive + DDB Global Tables. AWS-side DR (paired with R20's Pi-side data sync). **EFFORT: M / RISK: MED**
