...
try {
    ...
    Write-Host " [FAIL] Server not available" -ForegroundColor Red
...
try {
    ...
    Write-Host " [FAIL] Dashboard access failed" -ForegroundColor Red
}
...
try {
    ...
    if ($sessionData.authenticated -and $sessionData.role -eq "admin") {
        Write-Host " [OK] Session valid" -ForegroundColor Green
        Write-Host "   User: $($sessionData.user.email)" -ForegroundColor Gray
    } else {
        Write-Host " [FAIL] Session invalid" -ForegroundColor Red
    }
} catch {
    Write-Host " [FAIL] Session check failed" -ForegroundColor Red
}

Write-Host "`nQuick test completed!" -ForegroundColor Green
Write-Host "Run the complete test with: powershell -ExecutionPolicy Bypass -File test-admin-auth-complete-fixed.ps1" -ForegroundColor Cyan
