...
const updateCardWidth = () => {
  if (process.client) {
    cardWidth.value = window.innerWidth < 480 ? '100%' : '400';
  }
};
...
async function handleLogin() {
  ...
  try {
    ...
    console.log('🎉 Authentication successful, starting redirect...');
    
    // Enhanced navigation with multiple fallback strategies and better error handling
    setTimeout(async () => {
      try {
        debugInfo.value = 'Attempting navigation...';
        console.log('� Starting enhanced navigation to /users/...');
        
        // Strategy 1: Use Vue Router directly for better control
        const router = useRouter();
        console.log('�🔄 Method 1: Using Vue Router push...');
        await router.push('/users/');
        
        // Add delay to allow route change to process
        await new Promise(resolve => setTimeout(resolve, 300));
        
        // Strategy 2: Check if navigation worked
        if (process.client && window.location.pathname !== '/users/') {
          console.log('🔄 Vue Router failed, trying navigateTo...');
          debugInfo.value = 'Trying Nuxt navigation...';
          ...
          if (window.location.pathname !== '/users/') {
            console.log('🔄 navigateTo failed, using window.location...');
            debugInfo.value = 'Using direct navigation...';
            window.location.href = '/users/';
          } else {
            console.log('✅ Navigation successful via navigateTo');
            debugInfo.value = 'Navigation successful!';
          }
        } else {
          console.log('✅ Navigation successful via Vue Router');
          debugInfo.value = 'Navigation successful!';
        }
        
      } catch (navError) {
        console.error('❌ Navigation error:', navError);
        debugInfo.value = 'Trying final fallback...';
        
        try {
          if (process.client) {
            console.log('🔄 All strategies failed, forcing page reload...');
            window.location.replace('/users/');
          }
        } catch (windowError) {
          console.error('❌ All navigation methods failed:', windowError);
          debugInfo.value = 'Navigation failed - please manually go to /users/';
        }
      }
    }, 500); // Short delay to ensure auth state is updated
  } catch (err) {
