<template>
  <v-card class="ma-4">
    ...
    <v-card-text>
      <div v-if="!session">
        <v-alert type="info" variant="tonal"> No active training session </v-alert>
      </div>
          ...
          <v-col cols="12" md="6">
            <v-card variant="outlined">
              <v-card-text>
                ...
                <v-chip :color="getStatusColor(session.status)" variant="flat" class="mb-2">
                  <v-icon start>{{ getStatusIcon(session.status) }}</v-icon>
                  {{ session.status?.toUpperCase() || 'UNKNOWN' }}
                </v-chip>
                <div class="text-body-2">Started: {{ formatDate(session.created_at) }}</div>
          ...
          <v-col cols="12" md="6">
            <v-card variant="outlined">
              <v-card-text>
                ...
                <div class="text-body-2">Epoch {{ currentEpoch }} of {{ totalEpochs }}</div>
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
            ...
            <v-card variant="outlined">
              <v-card-text>
                ...
                <div class="text-h4 text-primary">
                  {{ getMetrics(session.metrics)?.loss?.toFixed(4) ?? 'N/A' }}
                </div>
                <div class="text-body-2 text-medium-emphasis">Training Loss</div>
              </v-card-text>
            </v-card>
          </v-col>
            ...
            <v-card variant="outlined">
              <v-card-text>
                ...
                <div class="text-h4 text-success">
                  {{
                    getMetrics(session.metrics)?.accuracy
                      ? (getMetrics(session.metrics).accuracy * 100).toFixed(2) + '%'
                      : 'N/A'
                  }}
                </div>
                <div class="text-body-2 text-medium-emphasis">Training Accuracy</div>
              </v-card-text>
            </v-card>
          </v-col>
            ...
            <v-card variant="outlined">
              <v-card-text>
                ...
                <div class="text-h4 text-warning">
                  {{ getMetrics(session.metrics)?.val_loss?.toFixed(4) ?? 'N/A' }}
                </div>
                <div class="text-body-2 text-medium-emphasis">Validation Loss</div>
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>

        <v-row v-if="getMetrics(session.metrics)?.history">
          <v-col cols="12">
            <v-card variant="outlined">
...
// Helper function to safely access metrics
const getMetrics = (metrics: any): any => {
...
const progress = computed(() => {
  ...
  const metrics = getMetrics(props.session.metrics);
  const current = metrics.epoch ?? 0;
  const total = metrics.total_epochs ?? 100;
...
const totalEpochs = computed(() => {
  const metrics = getMetrics(props.session?.metrics);
  return metrics.total_epochs ?? 100;
});
...
...
Current status: ${props.session.status}`;
    } catch (error) {
