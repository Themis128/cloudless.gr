...
export default async function globalSetup(_config: FullConfig): Promise<void> {
  ...
  await probe("/api/health", "application/json", (res, body) => {
    ...
    try {
      const json = JSON.parse(body) as { status?: string; dbConnected?: boolean };
      if (json.status !== "ok" || json.dbConnected !== true) {
        return `expected D1-bound health (status "ok", dbConnected true), got ${JSON.stringify(json)}`;
      }
    } catch {
