# Corrected GitHub Actions Runner Setup
# This script uses the proper GitHub API endpoints and token format

Write-Host "🚀 Corrected GitHub Actions Runner Setup" -ForegroundColor Green
...
# Test GitHub token
Write-Host "🔍 Testing GitHub token..." -ForegroundColor Blue
try {
    $headers = @{
    ...
    Write-Host "✅ User has access to repository: $($repoResponse.full_name)" -ForegroundColor Green
    
} catch {
    Write-Host "❌ Token validation failed: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "Please check your token permissions and try again" -ForegroundColor Yellow
    exit 1
}
...
# Create environment file with correct format
Write-Host "📝 Creating environment file..." -ForegroundColor Blue
$envContent = @"
GITHUB_TOKEN=$env:GITHUB_TOKEN
REPO_URL=https://github.com/$owner/$repo
RUNNER_NAME=cloudless-dev-runner
RUNNER_LABELS=linux,docker,dev,containerized,self-hosted
"@
...
docker logs cloudless-github-runner --tail 20 2>$null
