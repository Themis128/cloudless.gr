...
if isinstance(alerts, list):
    ...
    for a in alerts:
        lbls = a.get("labels", {})
        print(lbls.get("alertname","?"), "-", lbls.get("severity","?"),
...
esp = [x for x in targets
       if any(k in str(x.get("labels",{})).lower()
...
if not esp:
    ...
    for x in esp:
        print(x.get("health"), x.get("labels",{}).get("job"),
              x.get("labels",{}).get("instance"), x.get("lastError",""))

print("\n=== ESP32 metric names ===")
m = get(f"{PROM}/api/v1/label/__name__/values")
names = m.get("data", [])
esp_metrics = [n for n in names
               if any(k in n.lower()
                      for k in ["esp","sensor","temperature","humidity",
                                "watchdog","heartbeat","moisture"])]
