...
gql_query = (
    '{ repository(owner:"Themis128",name:"cloudless.gr"){ '
...
for b in branches:
    ...
    if committed < cutoff:
        stale.append(
            {"name": name, "days": (today - committed).days, "date": committed.date().isoformat()}
        )
...
if not stale:
    ...
    body = (
        f"## Stale branches (>21 days, no open PR)\n\n"
    ...
    issue = gh(
        ...
        data=json.dumps(
            {
                "title": f"chore: stale branch cleanup — {today_str}",
                "body": body,
                "labels": ["chore"],
            }
        ).encode(),
