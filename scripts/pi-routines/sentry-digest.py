...
def sentry_get(path, sentry_token):
    req = urllib.request.Request(
        f"https://sentry.io/api/0{path}", headers={"Authorization": f"Bearer {sentry_token}"}
    )
...
issues = sentry_get(
    f"/projects/{org}/{project}/issues/?query=is:unresolved&limit=25&statsPeriod=7d", sentry_token
)
...
if recurring:
    ...
    for i in recurring:
        ...
        lines.append(
            f":yellow_circle: `{i['title']}` — {i.get('count', '?')} events (since {first})"
        )
