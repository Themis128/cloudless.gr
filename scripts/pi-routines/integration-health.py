...
code, _ = api_check('https://api.stripe.com/v1/balance',
                    {'Authorization': f'Bearer {sk}'})
R.append((':green_circle:' if code == 200 else ':red_circle:', 'Stripe', f'HTTP {code}'))

# HubSpot
hk = ssm('HUBSPOT_API_KEY')
code, _ = api_check('https://api.hubapi.com/crm/v3/objects/contacts?limit=1',
                    {'Authorization': f'Bearer {hk}'})
R.append((':green_circle:' if code == 200 else ':red_circle:', 'HubSpot CRM', f'HTTP {code}'))

# Slack bot
code, data = api_check('https://slack.com/api/auth.test',
...
code, _ = api_check('https://api.notion.com/v1/users/me',
...
so = ssm('SENTRY_ORG')
code, _ = api_check(f'https://sentry.io/api/0/organizations/{so}/',
...
if ac_url and ac_tok:
    code, _ = api_check(f'{ac_url}/api/3/contacts?limit=1',
                        {'Api-Token': ac_tok})
    R.append((':green_circle:' if code == 200 else ':red_circle:', 'ActiveCampaign', f'HTTP {code}'))
else:
    R.append((':grey_question:', 'ActiveCampaign', 'SSM key not found'))

# Known-degraded (persistent, not actionable)
R.append((':yellow_circle:', 'Meta / Instagram', 'Blocked — ad policy violation (known, appeal pending)'))
R.append((':yellow_circle:', 'X (Twitter) Ads', 'Awaiting API approval (known)'))

# App health endpoint
code, _ = api_check('https://cloudless.gr/api/health')
R.append((':green_circle:' if code == 200 else ':red_circle:',
          'cloudless.gr /api/health', f'HTTP {code}'))
