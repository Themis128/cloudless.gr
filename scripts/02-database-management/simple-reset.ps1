...
Write-Host "🔄 Supabase Reset Script - IPv6 Compatible" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Gray
Write-Host ""

if (-not $Force) {
    Write-Host "⚠️  This will reset your local Supabase environment!" -ForegroundColor Yellow
...
if ($startResult -eq 0) {
    ...
    if (Test-Path "update-pgadmin-servers.ps1") {
        try {
            ...
            Write-Host "⚠️  pgAdmin update failed" -ForegroundColor Yellow
        }
    }
    ...
    if (-not $NoSeed -and (Test-Path "scripts/07-seed-database.js")) {
        ...
        try {
            ...
            Write-Host "⚠️  Seeding error: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
