...
# Fix the LangChain v1 structured-output experiment by simplifying it.
# The previous version mixed normal tools + ToolStrategy, which can cause local models
# to keep calling tools until LangGraph reaches GRAPH_RECURSION_LIMIT.

PROJECT_ROOT="${PROJECT_ROOT:-$HOME/code/cloudless.gr}"
TARGET="$PROJECT_ROOT/agents/experiments/langchain_v1_structured_output_local_vllm.py"
...
echo "✅ Rewrote and validated: $TARGET"
echo
echo "Run:"
echo "PYTHONPATH=. python agents/experiments/langchain_v1_structured_output_local_vllm.py"
