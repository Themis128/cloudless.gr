...
# Fix corrupted LangGraph local-dev deterministic text in cloudless.gr docs runner.
# Save as: ~/code/cloudless.gr/scripts/fix_corrupted_langgraph_runner.sh
# Run: cd ~/code/cloudless.gr && bash scripts/fix_corrupted_langgraph_runner.sh

PROJECT_ROOT="${PROJECT_ROOT:-$HOME/code/cloudless.gr}"
RUNNER="$PROJECT_ROOT/agents/run_langchain_docs_research.py"
...
cp "$RUNNER" "$RUNNER.bak-$(date +%Y%m%d-%H%M%S)"
...
echo "✅ Rewrote and validated: $RUNNER"
echo
printf 'Test with:\n'
