      ...
      <div>
        ...
        <div className="rounded-xl border border-yellow-900/30 bg-yellow-950/10 p-6">
          <p className="font-mono text-sm text-yellow-400">
            ...
            Note: Developer token approval from Google may take 1-5 days.
          </p>
        </div>
      </div>
    );
    ...
    <div>
      ...
      {metrics && (
        <div className="mb-8 grid grid-cols-5 gap-4">
          ...
          <MetricCard label="Spend" value={`$${(metrics.costMicros / 1_000_000).toFixed(2)}`} />
          <MetricCard label="Conversions" value={metrics.conversions.toFixed(1)} />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/50">
                ...
                <th className="px-4 py-3 text-left font-mono text-xs text-slate-500">Type</th>
              ...
              {campaigns.map((c) => (
                <tr key={c.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full border px-2 py-0.5 font-mono text-[10px] ${c.status === "ENABLED" ? "border-neon-green/30 text-neon-green" : "border-slate-700 text-slate-500"}`}>
                      {c.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-400">{c.advertisingChannelType}</td>
