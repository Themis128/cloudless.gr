...
/**
 * /admin/selfhosted — Self-hosted Apps Portal (R25)
 *
 * Dedicated one-click ingress launchpad for every self-hosted app on the Pi
 * cluster. Clicking "Open" calls /api/admin/autologin?app=<name> which:
 *   - For AppFlowy: does a server-side GoTrue password-grant and returns a
 *     token-injected deep-link so you land logged-in as admin.
 *   - For all others: returns the canonical admin URL (smart link).
 *
 * Each card also shows a live Kuma health badge fetched from the cluster
 * status endpoint so you know at a glance if an app is up before clicking.
 */
...
interface AppDef {
  ...
  icon: string;
  authMode: AuthMode;
  /** Hint text shown under the description */
  detail: string;
}
...
// ---------------------------------------------------------------------------
// App registry — ordered by daily-use frequency
// ---------------------------------------------------------------------------
...
const APPS: AppDef[] = [
  ...
  },
  {
    ...
    icon: "📝",
    authMode: "auto",
    detail: "Auto-login via GoTrue token — lands you directly in the workspace",
  },
  {
    key: "n8n",
    name: "n8n",
    description: "Workflow automation · Lead enrichment",
    icon: "🔀",
    authMode: "link",
    detail: "Trigger and inspect automation workflows",
  },
  {
    key: "postiz",
    name: "Postiz",
    description: "Social publishing · Scheduling",
    icon: "📨",
    authMode: "link",
    detail: "Schedule and review posts across connected social channels",
  },
  {
    key: "grafana",
    name: "Grafana",
  ...
  },
  {
    key: "kuma",
    name: "Uptime Kuma",
    description: "Uptime · Heartbeats · Status pages",
    icon: "🟢",
    authMode: "link",
    detail: "Check monitor status and configure heartbeat alerts",
  },
];
...
const AUTH_BADGE: Record<AuthMode, { label: string; klass: string }> = {
  auto: {
    label: "AUTO-LOGIN",
    klass: "border-neon-cyan/30 bg-neon-cyan/10 text-neon-cyan",
  },
  link: {
    label: "DIRECT LINK",
    klass: "border-slate-700 bg-slate-800/50 text-slate-400",
  },
  vpn: {
    label: "VPN ONLY",
    klass: "border-yellow-500/30 bg-yellow-500/10 text-yellow-400",
  },
};
...
/** Map Kuma monitor names to app keys — loose substring match */
  ...
  useEffect(() => {
    async function loadHealth() {
      try {
        ...
        /* silent — health badges just stay "unknown" */
      } finally {
        setKumaLoaded(true);
      }
    }
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="font-body mt-2 text-sm text-slate-400">
          One-click admin ingress to every app running on the Pi cluster. AppFlowy logs you in
          automatically via a server-side token. All other apps open at the admin URL.
        </p>
          ...
          <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-slate-800 bg-slate-900/40 px-3 py-1.5">
            <span
              className={`h-1.5 w-1.5 rounded-full ${
                anyDown ? "animate-pulse bg-red-400" : "bg-neon-green"
              }`}
            />
      ...
      <div className="mt-8 rounded-xl border border-slate-800 bg-slate-900/30 p-4">
        <p className="font-mono text-[10px] text-slate-500">
          ...
          on every click — do not bookmark the resulting URL.{" "}
          <span className="text-yellow-400">VPN ONLY</span> apps require Tailscale or direct access
          to the cluster network. Health badges are sourced from Uptime Kuma via{" "}
          <code className="text-slate-400">/api/admin/cluster/kuma-status</code>.
        </p>
      </div>
    </div>
  );
