...
function StatusDot({ ok }: { ok: boolean | null }) {
  ...
  return <span className={`h-2 w-2 rounded-full ${ok ? "animate-pulse bg-emerald-400" : "bg-red-500"}`} />;
}
  ...
  return (
    <div className={`flex gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      <div className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${
        ...
        : isInterrupt ? "bg-yellow-700 text-yellow-200"
        : msg.error ? "bg-red-900 text-red-300"
      ...
      <div className={`max-w-[82%] flex flex-col ${isUser ? "items-end" : "items-start"}`}>
        <div className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
          isUser ? "rounded-tr-sm bg-indigo-600 text-white"
          : isInterrupt ? "rounded-tl-sm bg-yellow-900/30 text-yellow-300 ring-1 ring-yellow-700/40"
          : msg.error ? "rounded-tl-sm bg-red-900/30 text-red-300 ring-1 ring-red-800/50"
          : "rounded-tl-sm bg-zinc-800 text-zinc-100"
        }`}>
          <pre className="whitespace-pre-wrap font-sans">{msg.content}</pre>
          {msg.streaming && <span className="ml-0.5 inline-block h-3.5 w-0.5 animate-pulse bg-current opacity-70" />}
        </div>
        {msg.runId && <span className="mt-0.5 font-mono text-[9px] text-zinc-700">run:{msg.runId.slice(0, 8)}</span>}
      </div>
    </div>
  );
}
...
function StorePanel() {
  ...
  async function doGet() {
    ...
    } catch (e) { setResult(`Error: ${e instanceof Error ? e.message : e}`); }
  ...
  async function doSearch() {
    setBusy(true); setResult(null);
    try {
      const r = await api("store-search", { namespace_prefix: [ns], query: key || undefined, limit: 10 });
      const d = await r.json();
      setResult(JSON.stringify(d, null, 2));
    } catch (e) { setResult(`Error: ${e instanceof Error ? e.message : e}`); }
    finally { setBusy(false); }
  ...
  return (
    <div className="space-y-3">
      <p className="text-[10px] uppercase tracking-wider text-zinc-600">Cross-thread Store</p>
      <div className="space-y-2">
        <input value={ns} onChange={e => setNs(e.target.value)}
          placeholder="namespace"
          className="w-full rounded bg-zinc-800 px-2 py-1 text-xs text-zinc-300 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-violet-600" />
        ...
        <textarea value={value} onChange={e => setValue(e.target.value)}
          ...
          rows={2}
          className="w-full resize-none rounded bg-zinc-800 px-2 py-1 text-xs text-zinc-300 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-violet-600" />
        <div className="flex gap-1">
          {(["Get", "Put", "Del", "Search"] as const).map((label) => {
            const fn = label === "Get" ? doGet : label === "Put" ? doPut : label === "Del" ? doDel : doSearch;
            return (
              <button key={label} onClick={() => void fn()} disabled={busy}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-[10px] uppercase tracking-wider text-zinc-600">Run history</p>
        <button onClick={() => void load()} disabled={busy}
          className="text-[10px] text-zinc-600 hover:text-zinc-400">↺</button>
      </div>
...
export default function LangGraphPage() {
  ...
  async function switchThread(id: string) {
    ...
    try {
      ...
      if (state.next?.length) {
        msgs.push({ id: uid(), role: "interrupt", content: `⏸ Paused before: ${state.next.join(", ")}` });
  ...
  async function deleteThread(e: React.MouseEvent, id: string) {
    e.stopPropagation();
    try { await api("delete-thread", { thread_id: id }); } catch { /* best effort */ }
  ...
  const send = useCallback(async (overrideText?: string) => {
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            } else if (streamMode === "updates" && typeof parsed === "object" && parsed !== null) {
              const updates = parsed as Record<string, { messages?: Array<{ content?: string }> }>;
      ...
      if (interrupted) {
        ...
        setMessages(prev => [...prev, { id: uid(), role: "interrupt", content: "⏸ Run paused — waiting for input" }]);
      }
    } catch (err) {
  ...
  async function sendResume() {
    ...
    try {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (Array.isArray(parsed)) {
              const chunk = (parsed as Array<Record<string, unknown>>)[0];
              if (chunk?.content && typeof chunk.content === "string") latestContent = chunk.content;
            }
            if (latestContent) setMessages(prev => prev.map(m => m.id === asstMsg.id ? { ...m, content: latestContent } : m));
          } catch { /* skip */ }
        }
      }
      setInterruptedRunId(null);
      ...
      const msg = err instanceof Error ? err.message : String(err);
      setMessages(prev => prev.map(m => m.id === asstMsg.id ? { ...m, content: msg, streaming: false, error: true } : m));
    } finally { setBusy(false); }
  }
      ...
      <aside className="flex w-64 shrink-0 flex-col border-r border-zinc-800 bg-zinc-900/80">
        <div className="flex items-center gap-2.5 border-b border-zinc-800 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-[10px] text-zinc-500">
              {assistant ? `${assistant.graph_id} · ${assistant.assistant_id.slice(0, 8)}` : isOffline ? "Server offline" : "…"}
            </p>
          </div>
          <button onClick={() => void init()} className="text-zinc-600 hover:text-zinc-300" title="Refresh">↺</button>
        </div>
          ...
          {(["chat", "store", "runs"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-1.5 text-[10px] uppercase tracking-wider transition-colors ${
                activeTab === tab ? "border-b-2 border-violet-500 text-violet-300" : "text-zinc-600 hover:text-zinc-400"
              }`}>
              {tab}
            </button>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {activeTab === "chat" && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase tracking-wider text-zinc-600">Threads</span>
                <button onClick={() => void newThread()} disabled={isOffline}
                  className="rounded px-1 text-xs text-zinc-500 hover:bg-zinc-700 hover:text-zinc-200 disabled:opacity-40">＋</button>
              </div>
              <div className="space-y-0.5">
                {threads.length === 0 && <p className="text-[11px] text-zinc-700">No threads yet</p>}
                {threads.map(t => (
                  <button key={t.thread_id} onClick={() => void switchThread(t.thread_id)}
                    className={`group flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-left text-xs transition-colors ${
                      t.thread_id === activeThreadId ? "bg-violet-900/50 text-white" : "text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200"
                    }`}>
                    <span className="truncate">{t.label ?? `Thread ${t.thread_id.slice(0, 8)}`}</span>
                    <span onClick={e => void deleteThread(e, t.thread_id)}
                      className="ml-1 shrink-0 rounded p-0.5 text-zinc-600 opacity-0 hover:bg-zinc-600 hover:text-zinc-300 group-hover:opacity-100">✕</span>
                  </button>
                ))}
              </div>

              <div className="pt-2">
                <p className="mb-1 text-[10px] uppercase tracking-wider text-zinc-600">Stream mode</p>
                <select value={streamMode} onChange={e => setStreamMode(e.target.value as StreamMode)}
                  className="w-full rounded bg-zinc-800 px-2 py-1 text-xs text-zinc-300 focus:outline-none focus:ring-1 focus:ring-violet-600">
              ...
              <div className="pt-1">
                <p className="mb-1.5 text-[10px] uppercase tracking-wider text-zinc-600">Tools</p>
                {[["📊","Analytics"],["📄","Top pages"],["🔑","Keywords"],["👥","Leads"],
                  ["📝","Projects"],["📈","KPIs"],["🖥","Cluster"],["📬","Subscriptions"]
                ].map(([i, l]) => (
                  <div key={l} className="flex items-center gap-1.5 py-0.5 text-[11px] text-zinc-600">
                    <span>{i}</span>{l}
                  </div>
                ))}
              </div>
        ...
        {isOffline && (
          <div className="border-t border-zinc-800 p-3">
            ...
            <code className="block break-all text-[9px] text-zinc-700">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto px-6 py-6">
          {messages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <p className="mb-6 text-xs text-zinc-600">Ask about analytics, leads, projects, or infrastructure.</p>
              <div className="grid grid-cols-2 gap-2 max-w-lg">
                {SUGGESTIONS.map(s => (
                  <button key={s} onClick={() => void send(s)} disabled={isOffline}
          ...
          <div className="border-t border-yellow-900/40 bg-yellow-950/20 px-6 py-3">
            ...
            <div className="flex gap-2">
              <input value={resumeInput} onChange={e => setResumeInput(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter") void sendResume(); }}
                placeholder="Resume value (e.g. 'yes', JSON, or any text)…"
                className="flex-1 rounded bg-zinc-800 px-3 py-1.5 text-sm text-zinc-200 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-yellow-600" />
              <button onClick={() => void sendResume()} disabled={busy || !resumeInput.trim()}
                className="rounded bg-yellow-700 px-3 py-1.5 text-xs text-white hover:bg-yellow-600 disabled:opacity-40">
                Resume
              </button>
              <button onClick={() => setInterruptedRunId(null)}
        ...
        <div className="border-t border-zinc-800 bg-zinc-900/60 px-6 py-4">
          <div className="mx-auto max-w-2xl">
            <div className={`flex items-end gap-3 rounded-2xl border bg-zinc-800 px-4 py-3 transition-colors ${
              isOffline ? "border-zinc-800 opacity-40" : "border-zinc-700 focus-within:border-violet-600/60"
            }`}>
              <textarea ref={inputRef} value={input}
                onChange={e => setInput(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void send(); } }}
                placeholder={isOffline ? "LangGraph server offline…" : `Ask anything… (mode: ${streamMode})`}
                disabled={isOffline || busy} rows={1}
              ...
              <button onClick={() => void send()} disabled={!input.trim() || isOffline || busy}
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-violet-600 text-white hover:bg-violet-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                {busy ? (
            ...
            <p className="mt-1.5 text-center text-[10px] text-zinc-700">
              ...
              {activeThreadId && <span className="ml-2 font-mono text-zinc-800">thread:{activeThreadId.slice(0, 8)}</span>}
            </p>
          </div>
        </div>
      </div>
