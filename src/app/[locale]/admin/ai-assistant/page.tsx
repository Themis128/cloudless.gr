...
export default function AIAssistantPage() {
  ...
  async function generateStrategy(e: React.FormEvent) {
    ...
    try {
      ...
      const data = (await res.json()) as { strategy: Record<string, unknown> };
            ...
            <div className="space-y-3">
              {copyVariants.map((v, i) => (
                ...
                >
                  ...
                  {v.headline && (
                    <p className="text-neon-cyan mb-1 font-mono text-sm font-semibold">
                      {v.headline}
                    </p>
                  )}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {platforms.map((p) => (
              <span
                key={p}
                className="border-neon-cyan/30 text-neon-cyan rounded-full border px-2 py-0.5 font-mono text-xs"
              >
                {p}
              </span>
            ))}
