      ...
      {loading ? (
        ...
        seoError ? (
          <div className="bg-void-light/50 rounded-xl border border-red-900/30 p-6 text-center">
            ...
            <p className="mt-2 text-xs text-slate-500">
              Set AHREFS_API_KEY in your environment.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-5">
              {[
                {
                  label: "Domain Rating",
                  value: seo?.snapshot?.domainRating ?? "—",
                  accent: "neon-magenta",
                },
                {
                  label: "Organic Keywords",
                  value:
                    seo?.snapshot?.organicKeywords?.toLocaleString() ?? "—",
                  accent: "neon-cyan",
                },
                {
                  label: "Organic Traffic",
                  value: seo?.snapshot?.organicTraffic?.toLocaleString() ?? "—",
                  accent: "neon-green",
                },
                {
                  label: "Backlinks",
                  value: seo?.snapshot?.backlinks?.toLocaleString() ?? "—",
                  accent: "neon-blue",
                },
                {
                  label: "Ref. Domains",
                  value:
                    seo?.snapshot?.referringDomains?.toLocaleString() ?? "—",
                  accent: "neon-magenta",
                },
              ].map((stat) => (
                <div
                    ...
                    {(seo?.topKeywords ?? []).map((kw, i) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`font-mono text-xs ${kw.position <= 3 ? "text-neon-green" : kw.position <= 10 ? "text-neon-cyan" : "text-slate-400"}`}
                          >
                            #{kw.position}
                          </span>
                        </td>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
          {[
            {
              label: "Pageviews",
              value: web?.analytics?.pageviews?.toLocaleString() ?? "—",
            },
            {
              label: "Visitors",
              value: web?.analytics?.visitors?.toLocaleString() ?? "—",
            },
            {
              label: "Visits",
              value: web?.analytics?.visits?.toLocaleString() ?? "—",
            },
            {
              label: "Bounce Rate",
              value:
                web?.analytics?.bounceRate != null
                  ? `${(web.analytics.bounceRate * 100).toFixed(1)}%`
                  : "—",
            },
            {
              label: "Avg Duration",
              value:
                web?.analytics?.avgDuration != null
                  ? `${Math.round(web.analytics.avgDuration)}s`
                  : "—",
            },
          ].map((stat) => (
            <div
              key={stat.label}
              className="bg-void-light/50 rounded-xl border border-slate-800 p-6 text-center"
            >
              <p className="font-mono text-[10px] text-slate-500">
                {stat.label}
              </p>
