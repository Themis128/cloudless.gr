...
function renderBlock(block: string, keyPrefix: number) {
  ...
  if (isList) {
    ...
    return (
      <ul key={keyPrefix} className="mt-3 list-disc space-y-2 pl-5 text-slate-300">
        {items.map((item, i) => (
  ...
  return (
    <p key={keyPrefix} className="mt-3 leading-relaxed text-slate-300">
      {renderInline(block)}
    </p>
  );
}
...
export async function generateStaticParams() {
  ...
  const allSlugs = new Set([...posts.map((p) => p.slug), ...staticPosts.map((p) => p.slug)]);
