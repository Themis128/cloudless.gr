...
async function persistPortals(portals: ClientPortal[]): Promise<NextResponse | null> {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to save client portals." },
      { status: 502 }
    );
  }
}
...
=======
    const withHealth = portals.map((p) => ({
      ...p,
      health: scoreClientHealth(p, new Date()),
    }));
>>>>>>> Stashed changes
    ...
    return NextResponse.json(
      { error: "Failed to load client portals." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const explicitWorkspaceId = body.workspaceId;
  const workspaceId =
...
=======
      : await getActiveWorkspaceId(request);
>>>>>>> Stashed changes
...
function applyAddDeliverable(
  ...
  if (!body.title?.trim()) {
    return NextResponse.json(
      { error: "Deliverable title required" },
      { status: 400 }
    );
  }
  portal.deliverables ??= [];
...
function dispatchPatch(portal: ClientPortal, body: PatchAction): NextResponse | null {
  switch (body.action) {
    ...
    case "delete-deliverable":
      portal.deliverables = (portal.deliverables ?? []).filter(
        (d) => d.id !== body.deliverableId
      );
      return null;
    ...
    case "delete-payment-link":
      portal.paymentLinks = (portal.paymentLinks ?? []).filter(
        (l) => l.id !== body.paymentLinkId
      );
      return null;
