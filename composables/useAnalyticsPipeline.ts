...
export const useAnalyticsPipeline = () => {
  ...
  // Analytics Pipelines
  const createPipeline = async (pipeline: AnalyticsPipelineInsert): Promise<AnalyticsPipeline | null> => {
    const { data, error } = await supabase
      .from('analytics_pipelines')
      .insert({ ...pipeline, owner_id: user.value?.id })
      .select()
      .single()
  ...
  const updatePipeline = async (id: string, updates: AnalyticsPipelineUpdate): Promise<AnalyticsPipeline | null> => {
    const { data, error } = await supabase
      .from('analytics_pipelines')
  ...
  const updateStep = async (id: string, updates: PipelineStepUpdate): Promise<PipelineStep | null> => {
    const { data, error } = await supabase
      .from('pipeline_steps')
      .update({ ...updates, updated_at: new Date().toISOString() })
      .eq('id', id)
  ...
  const reorderSteps = async (pipelineId: string, stepOrders: Array<{ id: string; position: number }>): Promise<boolean> => {
