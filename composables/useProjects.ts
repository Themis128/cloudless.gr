...
import type { Project, ProjectFilter, ProjectCategory, ProjectStatus } from '~/types/projects';
      ...
      return projects.value
        .filter(p => p.id !== project.id && 
                    (p.category === project.category || 
                     p.tech_tags.some(tag => project.tech_tags.map(t => t.tag_name).includes(tag.tag_name))))
        .slice(0, 3);
    }
