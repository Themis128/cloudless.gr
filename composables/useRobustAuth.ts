...
export const useRobustAuth = () => {
  ...
  const checkAccountLock = async (email: string): Promise<boolean> => {
    try {
      const { data, error } = await supabase.rpc('is_user_locked', {
        user_email: email
      })
  ...
  const requestPasswordReset = async (email: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      const { error: resetError } = await supabase.auth.resetPasswordForEmail(email, {
        redirectTo: `${window.location.origin}/auth/reset-password?token=${token}`
      })
  ...
  const resetPassword = async (token: string, newPassword: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      // Verify reset token
      const { data: verification, error: verifyError } = await supabase.rpc('verify_reset_token', {
        token
      })
      
      if (verifyError || !verification || verification.length === 0 || !verification[0].is_valid) {
        throw new Error('Invalid or expired reset token')
      }
      
      const { user_id, email } = verification[0]
      ...
      // Clear reset token
      await supabase.rpc('clear_reset_token', {
        user_email: email
      })
