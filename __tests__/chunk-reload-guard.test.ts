...
describe("ChunkReloadGuard — CHUNK_ERROR_PATTERN", () => {
  it("matches known chunk-load failure messages", () => {
    expect(
      CHUNK_ERROR_PATTERN.test("ChunkLoadError: Loading chunk 5 failed"),
    ).toBe(true);
    expect(CHUNK_ERROR_PATTERN.test("Loading chunk 0q2ljv8a failed")).toBe(
      true,
    );
    expect(
    ...
    expect(
      CHUNK_ERROR_PATTERN.test("Failed to fetch dynamically imported module"),
    ).toBe(true);
  });
  ...
  it("does not match unrelated runtime errors", () => {
    ...
    expect(CHUNK_ERROR_PATTERN.test("ReferenceError: foo is not defined")).toBe(
      false,
    );
