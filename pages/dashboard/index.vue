<template>
  <v-container fluid class="pa-4" style="max-width: 1200px; margin: 0 auto">
    <!-- Loading State -->
    <v-row v-if="!isUserDataLoaded" justify="center" class="my-12">
...
<script setup lang="ts">
  ...
  const loadUserData = async () => {
    try {
      ...
      // Load user stats from localStorage or use defaults (only on client)
      if (process.client) {
        const savedStats = localStorage.getItem('userStats');
        if (savedStats) {
          try {
            const parsedStats = JSON.parse(savedStats);
            userStats.value = { ...userStats.value, ...parsedStats };
          } catch (error) {
            console.warn('Could not parse saved user stats:', error);
          }
        }
      }
  ...
  // Save user stats to localStorage when they change (only on client)
  const saveUserStats = () => {
    if (process.client) {
      try {
        localStorage.setItem('userStats', JSON.stringify(userStats.value));
      } catch (error) {
        console.warn('Could not save user stats to localStorage:', error);
      }
    }
  };
  // Lifecycle hooks
