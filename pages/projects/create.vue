<template>
  ...
  <v-dialog v-model="showOnboarding" max-width="500" persistent>
    <v-card>
      ...
      <v-card-text>
        This wizard will guide you through creating and managing your project step by step.<br>
        <ul>
          <li>Follow the steps below and use the navigation buttons.</li>
          <li>Click the help icon <v-icon small color="primary">mdi-help-circle</v-icon> for more info on each step.</li>
        </ul>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
        <v-btn color="primary" @click="showOnboarding = false">Get Started</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <div class="mb-2 text-right">
    <span class="text-caption">Step {{ wizard.currentStep.value + 1 }} of {{ wizard.stepCount }}</span>
  </div>
  <v-container>
    <v-row justify="center">
      <v-col cols="12" md="8" lg="6">
        <v-card>
          <v-card-title>
            Create Project
            <v-btn icon size="small" class="ml-2" @click="showHelp = true" aria-label="Help"><v-icon>mdi-help-circle</v-icon></v-btn>
          </v-card-title>
          <v-card-text>
            <div class="mb-4">
              ...
              <v-code>
{
  ...
  "description": "Project description"
            ...
            <v-form @submit.prevent="onSubmit" ref="formRef" aria-label="Project creation form">
              <v-text-field
                v-model="name"
                label="Name"
              ...
              />
              <div id="name-desc" class="text-caption mb-2">Project name must be unique and descriptive.</div>
              <v-text-field
                v-model="description"
                label="Description"
              ...
              />
              <div id="desc-desc" class="text-caption mb-2">Describe the purpose of your project (optional).</div>
              <v-row class="mt-4">
                <v-col cols="6">
                ...
                <v-col cols="6" class="text-right">
                  <v-btn color="primary" variant="elevated" :disabled="!canProceed" @click="wizard.isLastStep ? onSubmit() : wizard.next">
            ...
            <v-dialog v-model="showHelp" max-width="400">
              <v-card>
                ...
                <v-card-text>
                  Enter a unique project name and (optionally) a description. Use the navigation buttons to move between steps. For more info, see the documentation.
                </v-card-text>
                <v-card-actions>
                  <v-spacer />
            ...
            <v-alert v-if="success" type="success" class="mt-3" aria-live="polite">Project created!</v-alert>
