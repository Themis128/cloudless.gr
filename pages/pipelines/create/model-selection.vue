        ...
        <v-card>
          ...
          <v-card-text>
            <v-form @submit.prevent="saveModelSelection">
              ...
              <v-radio-group v-model="modelType" inline class="mb-4">
                <v-radio label="Predefined Model" value="predefined" />
                <v-radio label="Custom Model" value="custom" />
              </v-radio-group>
              ...
              <!-- Model Parameters -->
              <v-expansion-panels
                v-if="selectedModel || modelType === 'custom'"
              >
                <v-expansion-panel>
                  <v-expansion-panel-title
                    >Model Parameters</v-expansion-panel-title
                  >
                  <v-expansion-panel-text>
                    ...
                    >
                      <template #prepend>
                        <v-tooltip
                          location="top"
                          text="Controls the randomness in model outputs. Lower values (closer to 0) make the model more focused and deterministic, while higher values (closer to 1) make it more creative and diverse."
                        >
                    ...
                    >
                      <template #prepend>
                        <v-tooltip
                          location="top"
                          text="Maximum number of tokens (words or word pieces) in the model's response. One token is roughly 4 characters or 3/4 of a word in English. Higher values allow for longer responses but may increase processing time and costs."
                        >
                          <template #activator="{ props }">
                            <v-icon
                              v-bind="props"
                      ...
                      <template #append>
                        <v-tooltip
                          location="top"
                          text="Estimated maximum words: ~{{ Math.round(modelParams.maxTokens * 0.75) }}"
                        >
                          <template #activator="{ props }">
                            <v-icon
                              v-bind="props"
                    ...
                    >
                      <template #prepend>
                        <v-tooltip
                          location="top"
                          text="The format in which the model will structure its response. Choose based on how you plan to process or display the output."
                        >
                          <template #activator="{ props }">
                            <v-icon
                              v-bind="props"
...
const outputFormats = [
  {
    value: 'json',
    title: 'JSON',
    description: 'Structured data format, best for parsing and data processing',
  },
  {
    value: 'text',
    title: 'Plain Text',
    description: 'Simple text output without formatting',
  },
  {
    value: 'markdown',
    title: 'Markdown',
    description: 'Rich text with formatting, headers, lists, and code blocks',
  },
  {
    value: 'html',
    title: 'HTML',
    description: 'Web-ready format with HTML tags and styling',
  },
]
